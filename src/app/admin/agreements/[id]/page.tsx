...
export default function AgreementDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  // Notify Guarantor State
  const [showNotifyModal, setShowNotifyModal] = useState(false);
  const [notifySubject, setNotifySubject] = useState('');
  const [notifyMessage, setNotifyMessage] = useState('');
  const [notifying, setNotifying] = useState(false);
  const [notifyFeedback, setNotifyFeedback] = useState<{type: 'error' | 'success', text: string} | null>(null);

  function openNotifyModal() {
    if (!agreement) return;
    setNotifySubject(`Account Update: ${agreement.hirer.name} - ${agreement.vehicle.registrationNo}`);
    setNotifyMessage(`Hello Guarantor,\n\nThis is an update regarding the hire-purchase agreement for ${agreement.hirer.name}.\n\nCurrent Balance: GHS ${agreement.summary.balanceRemaining.toFixed(2)}\nStatus: ${agreement.summary.statusBadge.label}\n\nPlease contact us for further details.\n\nThank you.`);
  ...
  async function handleNotifySubmit(e: React.FormEvent) {
    ...
    setNotifyFeedback(null);
    try {
      const res = await fetch(`/api/agreements/${resolvedParams.id}/notify-guarantor`, {
        method: 'POST',
      ...
      setNotifyFeedback({ type: 'success', text: 'Email notification sent successfully!' });
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-6 pb-12">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          <button
            ...
            className="bg-indigo-600 hover:bg-indigo-500 text-white font-semibold text-xs px-3.5 py-2 rounded-xl shadow-xs inline-flex items-center gap-1.5 transition-colors"
          >
            <Mail className="w-4 h-4" />
            <span>Notify Guarantor</span>
          </button>
            ...
            <form onSubmit={handleStatusChangeSubmit} className="space-y-4">
              ...
              <div className="flex items-center justify-end gap-2 border-t border-slate-100 pt-3">
                ...
                <button
                  ...
                  className="px-4 py-2 text-xs font-bold text-white bg-slate-900 hover:bg-slate-800 rounded-lg shadow-sm transition-colors disabled:opacity-50"
                >
      ...
      {/* Notify Guarantor Modal */}
      {showNotifyModal && (
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-xs flex items-center justify-center p-4 z-50">
          <div className="bg-white rounded-2xl max-w-lg w-full p-6 shadow-2xl space-y-4 border border-slate-200">
            <div className="flex items-center justify-between border-b border-slate-100 pb-3">
              <h3 className="font-bold text-slate-900 text-base flex items-center gap-2">
                <Mail className="w-5 h-5 text-indigo-500" /> Notify Guarantor
              </h3>
              <button
            ...
            <form onSubmit={handleNotifySubmit} className="space-y-4">
              <div>
                ...
                <input
                  ...
                  className="w-full bg-slate-50 border border-slate-200 rounded-xl text-xs font-bold p-2.5 focus:ring-2 focus:ring-indigo-500 outline-none"
                />
              </div>
              ...
              <div>
                ...
                <textarea
                  ...
                  className="w-full bg-slate-50 border border-slate-200 rounded-xl text-xs p-3 focus:bg-white focus:ring-2 focus:ring-indigo-500 focus:outline-none font-sans"
                />
              </div>
              ...
              <div className="flex items-center justify-end gap-2 border-t border-slate-100 pt-3">
                ...
                <button
                  type="submit"
                  disabled={notifying}
                  className="px-4 py-2 text-xs font-bold text-white bg-indigo-600 hover:bg-indigo-500 rounded-lg shadow-sm transition-colors disabled:opacity-50 flex items-center gap-2"
                >
